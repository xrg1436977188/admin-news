import React, { useState, useEffect } from 'react'
import { Table, Button, Popconfirm, Space, Tag, message } from 'antd';
import { $newsAuditList, $byIdupdateNewsTwoStatus } from '@/api/news'
import dayjs from 'dayjs'
export default function AuditList() {
  const [tableList, setTableList] = useState([])
  const publish_status_list = ['未发布', '待发布', '已上线', '已下线']
  const audit_status_list = ['未审核', '审核中', '审核通过', '拒绝审核']
  const status_color_list = ['black', 'orange', 'green', 'red']
  useEffect(() => {

    newsAuditList()
  }, [])
  const newsAuditList = async () => {
    const res = await $newsAuditList()
    if (res.status === 0) {
      setTableList(res.data)
    }
  }
  const publishCancel = () => {
    message.error('取消发布')
  }
  const publishSubmit = async (id) => {
    const res = await $byIdupdateNewsTwoStatus({
      audit_status: 2,
      publish_status: 2,
      id
    })
    if (res.status === 0) {
      newsAuditList()
      message.success('发布成功,请在发布管理[已上线]中查看')
    }
  }
  const revoKe = async (id) => {
    const res = await $byIdupdateNewsTwoStatus({
      audit_status: 0,
      publish_status: 0,
      id
    })
    if (res.status === 0) {
      newsAuditList()
      message.success('撤销成功,请在草稿箱中查看')
    }
  }
  const revoKeCannel = () => {
    message.error('取消审核')
  }

  const columns = [
    {
      title: 'ID',
      dataIndex: 'id',
      key: 'id',
      align: 'center',
      width: 100,
    },
    {
      title: '文章标题',
      dataIndex: 'title',
      key: 'title',
      align: 'center',
      width: 150,
      render: (title, data) => (
        <a href={`/news_preview/${data.id}`}>{title}</a>
      )
    },
    {
      title: '分类',
      dataIndex: 'category',
      key: 'category',
      align: 'center',
      width: 150,
    },
    {
      title: '发布人',
      dataIndex: 'identity',
      key: 'identity',
      align: 'center',
      width: 150,
    },
    {
      title: '作者姓名',
      dataIndex: 'name',
      key: 'name',
      align: 'center',
      width: 150,

    },
    {
      title: '审核状态',
      dataIndex: 'audit_status',
      key: 'audit_status',
      align: 'center',
      width: 100,
      render: (audit_status) => (
        <Tag color={status_color_list[audit_status]}>
          {audit_status_list[audit_status]}
        </Tag>
      )

    },

    {
      title: '发布状态',
      dataIndex: 'publish_status',
      key: 'publish_status',
      align: 'center',
      width: 100,
      render: (publish_status) => (
        <Tag color={status_color_list[publish_status]}>
          {publish_status_list[publish_status]}
        </Tag>
      )

    },

    {
      title: '创建时间',
      dataIndex: 'create_time',
      key: 'create_time',
      align: 'center',
      width: 150,
      render: (create_time) => (
        <span>{dayjs(create_time).format('YYYY-MM-DD')}</span>
      )
    },
    {
      title: '更新时间',
      dataIndex: 'update_time',
      key: 'update_time',
      align: 'center',
      width: 150,
      render: (update_time) => (
        <span>{update_time ? dayjs(update_time).format('YYYY-MM-DD') : '-'}</span>
      )
    },
    {
      title: '操作',
      key: 'action',
      render: (data) => (
        <Space size="small">

          <Button disabled={data.audit_status === 2} style={{ marginRight: 20 }} type='primary' onClick={() => {
            window.location.href = `/layout/draft_box/news_edit/${data.id}`
          }}>更新</Button>
          <Popconfirm
            title="发布提示"
            description="你确定要发布吗?"
            onConfirm={() => publishSubmit(data.id)}
            onCancel={publishCancel}
            okText="确定"
            cancelText="取消"
          >
            <Button disabled={ data.publish_status===2||data.audit_status === 3||data.audit_status === 1} style={{ marginRight: 20 }} danger>发布</Button>
          </Popconfirm>

          <Popconfirm
            title="撤销提示"
            description="你确定要撤销吗?"
            onConfirm={() => revoKe(data.id)}
            onCancel={revoKeCannel}
            okText="确定"
            cancelText="取消"
          >
            <Button disabled={data.audit_status === 2 || data.audit_status === 3} type='primary' ghost>撤销</Button>
          </Popconfirm>

        </Space >
      ),
      fixed: 'right',
    },
  ];
  return (
    <div>
      <Table pagination={{pageSize:'6'}} rowKey={'id'} bordered size='small' dataSource={tableList} columns={columns} />
    </div>
  )
}
